...
### Fixed
- **小窗口右键菜单被裁切**：隐阅窗口高度过小时，进度条上的右键菜单会被窗口裁掉无法查看/点击；现在菜单弹出时会临时把窗口撑到能完整放下菜单的高度（底边不动、向上增长），关闭菜单后自动还原，撑高期间拖动/缩放不再把窗口缩回菜单放不下的高度
- **拖动封面误触发导入**：拖动书籍封面不再被识别为文件拖入，避免误弹"导入书籍"界面
- **打开文件位置误报失败**：仅在调用真正失败时提示
